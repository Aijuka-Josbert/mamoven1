    ...
    if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (isset($_POST['verify_code'])) {
            ...
            if (empty($reset_code) || empty($email)) {
                ...
                try {
                    // IMPORTANT: Use ORDER BY created_at DESC LIMIT 1 to get the most recent reset request
                    $stmt = $pdo->prepare("
            ...
            if (!isset($_SESSION['reset_user_id']) || !isset($_SESSION['reset_email'])) {
                ...
                try {
                    ...
                    // Delete all reset records for this email (clean up)
...
// Countdown timer for rate limit (if any)
<?php if ($rate_limit_retry_after > 0): ?>
(function() {
