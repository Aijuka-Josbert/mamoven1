...
function addToCart(productId, quantity = 1) {
  ...
  $.ajax({
    ...
    beforeSend: function () {
      button
        .html('<span class="loading"></span> Adding...')
        .prop("disabled", true);
    },
...
function renderStarsText(ratingValue) {
  const safeRating = Math.max(
    0,
    Math.min(5, Math.round(Number(ratingValue) || 0)),
  );
...
function beginEditReview(reviewId) {
  ...
  const targetReview = latestReviewData.reviews.find((review) => {
    return (
      Number(review.id) === Number(reviewId) && Number(review.can_edit) === 1
    );
  });
  ...
  if (submitBtn) {
    submitBtn.innerHTML =
      '<i class="fas fa-pen-to-square me-2"></i> Update Review';
  }
  ...
  $.ajax({
    ...
    error: function (xhr) {
      ...
      $("#reviews-container").html(
        '<p class="text-muted">' +
          (serverMessage || "Could not load reviews.") +
          "</p>",
      );
    },
  });
    ...
    reviewData.reviews.forEach(function (review) {
      ...
      const verifiedBadge =
        Number(review.is_verified_purchase) === 1
          ...
          : "";
      const ownBadge = canEdit
        ? '<span class="badge bg-info-subtle text-info border border-info-subtle ms-2 review-own-badge">Your review</span>'
        : "";
      const editedNote = reviewWasEdited(review)
        ? '<span class="review-edited-note">(edited)</span>'
        : "";
  ...
  if (reviewForm) {
    reviewForm.addEventListener("submit", function (e) {
      ...
      const reviewId = parseInt(
        document.getElementById("review-id-input")?.value || "0",
        10,
      );
      const submitBtn = document.getElementById("review-submit-btn");
